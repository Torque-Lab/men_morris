...
export class Board {
    ...
    public movePiece(from: Position, to: Position, color: Piece, isFlying: boolean = false): boolean {
        if (!this.positions.hasOwnProperty(from) || !this.positions.hasOwnProperty(to)) {
            return false;
        }
        if (this.positions[from] !== color || this.positions[to] !== null) {
            return false;
        }
    ...
    public getMillCombination(position: Position, color: Piece): Position[] {
        for (const mill of Board.MILL_COMBINATIONS) {
            if (mill.includes(position) && mill.every(pos => this.positions[pos] === color)) {
                return mill;
            }
        }
        return [];
